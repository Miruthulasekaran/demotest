...
export default function DeviceProfileEdit() {
    ...
    useEffect(() => {
        ...
        setId(idds)
        ApiService.DeviceProfileEdits(params.id).then((resp) => {
    ...
    useEffect(() => {
        setFinalArr({ created_by: "test", custom_data: Arr, device_profile_name: profile, updated_by: "TestUpdate" })
        // console.log("This is the useeffect data ", Arr)
    }, [Arr, profile]);
    // console.log("This is the normal data ", Paramvalue)
    let backendData = () => {
       if ( !/^(\w+\s?)*\s*$/g.test(profile)) {
            seterrorProfile1(true)
        }
        else if (profile.trim() == "") {
            seterrorProfile(true)
        }else{
            ApiService.deviceprofilePatch(params.id,finalArr).then((resp) => {
                ...
                if(resp.message){
                    ...
                    // alert("device profile name already exists !")
                }else{
        ...
        if (profile.trim() == "") {
            ...
            setArr(Arr.map((element) => {
                ...
                if (element.parameter === TempId) {
                    // console.log("mango", element)
                    return { ...element, parameter: parameter, data_type: drpDwn, profile: profile }
                }
                return element;
            }))
                                        ...
                                        <Row>
                                            ...
                                            <Col lg="5">
                                                <FormGroup>
                                                    ...
                                                    {errordataType ? <td style={{ color: "red" }}>Please select the data type</td> : null}
                                                </FormGroup>
                                            </Col>
                                            <Col lg="2" >
                                                <FormGroup >
