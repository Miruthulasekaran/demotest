...
export default function DeviceProfileAdd() {
    ...
    useEffect(() => {
        setFinalArr({ created_by: "test", updated_by: "TestUpdate", device_profile_name: profile, custom_data: Paramvalue })
        // console.log("This is the useeffect data ", Paramvalue)
    }, [Paramvalue,profile]);
    ...
    let backendData = () => {
        if (!/^(\w+\s?)*\s*$/g.test(profile)) {
            ...
            ApiService.DeviceProfileAdding(finalArr).then((resp) => {
                if (resp.message) {
                    console.log(resp);
                    ...
                    // toast("Device Profile added successfully!")
